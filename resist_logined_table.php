...
<html>
    ...
    $quarterKeyMap = array('1Q' => 'Quarter1', '2Q' => 'Quarter2', '3Q' => 'Quarter3', '4Q' => 'Quarter4');

    // 管理者がProf.jsonを一時的に編集している状態か確認する。
    $isProfessorEditing = isset($_SESSION['Prof_loginSuccess'])
        && $_SESSION['Prof_loginSuccess'] === true
        && isset($_SESSION['student_json_file'])
    ...
    if($code == HTTP_OK){
        ...
        if(isset($_SESSION['Student_login_Success']) && $_SESSION['Student_login_Success'] == true && $_SESSION['student_json_file'] !== 'Prof.json'){
            $selectedGrade = isset($_POST['grade']) && in_array($_POST['grade'], $gradeNames, true)
                ? $_POST['grade']
                : $studentData['grade'];
            echo '学年<br>';
        ...
        //確認画面から戻った場合はPOST値、それ以外はログイン済み情報を表示する。
        $emailValue = isset($_POST['email']) ? $_POST['email'] : $studentData['email'];
        echo 'メールアドレス';
        echo '<input type="text" name="email" value="'.htmlspecialchars($emailValue, ENT_QUOTES, 'UTF-8').'"><br>';

        $nameValue = isset($_POST['name']) ? $_POST['name'] : $studentData['name'];
        echo '氏名';
        echo '<input type="text" name="name" value="'.htmlspecialchars($nameValue, ENT_QUOTES, 'UTF-8').'"><br>';

        $passValue = isset($_POST['password']) ? $_POST['password'] : $studentData['password'];
        echo 'パスワード';
        ...
        echo '</div>';
        echo '<button type="submit" name="applyScheduleEvent" value="1" formaction="resist_logined_table.php">予定を配置</button>';
                ...
                for( $j = 0 ; $j < TABLE_LINE_COUNT ; $j++){ //月～土
                 ...
                 for( $j = 0 ; $j < TABLE_LINE_COUNT ; $j++){ //月～土
                    ...
                    if(isset($_POST['class'][$jsonDayNames[$j-1]][$i])){
                        ...
                        $quarterKey = $quarterKeyMap[$selectedQuarter];
                        $classValue = $studentData['class'][$quarterKey][$jsonDayNames[$j-1]][$i];
                        echo '<td><input type="text" id="cell_' . $jsonDayNames[$j-1] . '_' . $i . '" name="class[' . $jsonDayNames[$j-1] . '][' . $i . ']" value="'.htmlspecialchars($classValue, ENT_QUOTES, 'UTF-8').'"></td>';
                    }
        ...
        // POST値を使わず、保存済みJSONから画面全体を読み直す。
        echo '<a class="button reset-button" href="resist_logined_table.php?reset=1">保存済み内容に戻す</a>';
        echo '</form>';
        if($isProfessorEditing){
            echo '<a class="button reset-button" href="exit_prof_edit.php">管理者トップへ戻る</a>';
        }
        else{
            echo '<a class="button reset-button" href="logout.php">ログアウト</a>';
        }

        //クオーターを切り替えても入力内容を保持し、送信時は全クオーター分をまとめて送るためのJS
        //→ JSのquarterDataの初期値として、既存の登録内容(全学期分)をPHP側で組み立てておく
        ...
        foreach($quarterLabelMap as $label => $qKey){
            ...
            if(isset($_SESSION['Student_login_Success']) && $_SESSION['Student_login_Success'] == true){
                ...
                //万一セッション切れ等でログイン判定が外れていた場合は空欄で初期化
                $emptyDay = array();
