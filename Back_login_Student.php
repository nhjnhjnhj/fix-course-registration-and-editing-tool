<?php
    ...
    for($i = 0 ; $i < JSON_FILE_NUM ; $i++){
        ...
        for($j = 0 ; $j < count($data_Student) ; $j++){ //jsonファイルの配列の数だけループ
            ...
            if($data_Student[$j]['email'] == $_POST['email'] && $data_Student[$j]['name'] == $_POST['name'] && $data_Student[$j]['password'] == $_POST['password']){
                ...
                header('Location: resist_table.php'); //授業登録ページへ遷移
                exit();
                break;
            }
        }
    ...
    if($_SESSION[$Student_login_Success] == false){
